# Karpenter Instance Selection: A Technical Deep Dive
...
- [Phase 1: Scheduling and Bin-Packing](#phase-1-scheduling-and-bin-packing)
- [Phase 2: Launch Template Generation](#phase-2-launch-template-generation)
- [Phase 3: CreateFleet API Call](#phase-3-createfleet-api-call)
- [Allocation Strategies](#allocation-strategies)
- [Priority Values and Price Adjustments](#priority-values-and-price-adjustments)
- [How AWS Makes the Final Selection](#how-aws-makes-the-final-selection)
- [Real-World Examples](#real-world-examples)
- [Implications for Veneer NodeOverlay](#implications-for-veneer-nodeoverlay)
...
### Code Location
- **Repository**: `sigs.k8s.io/karpenter`
...
```go
// From cloudprovider/types.go:221-233
...
// From cloudprovider/types.go:322-327
...
### Code Location
- **File**: `pkg/providers/instance/types.go:209-217`
...
> From [EC2 Fleet Allocation Strategies](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-fleet-allocation-strategy.html):
> *"The fleet identifies the pools with the highest capacity availability for the number of instances that are launching... The fleet then requests Spot Instances from the lowest priced of these pools."*
...
> From AWS docs: *"EC2 Fleet uses priority on a best-effort basis to determine which launch template override to use in fulfilling Spot capacity, but optimizes for capacity first."*
...
> From [EC2 Fleet Allocation Strategies](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-fleet-allocation-strategy.html):
> *"The Spot Instances come from the lowest priced pool that has available capacity."*

#### `prioritized` (With NodeOverlay for On-Demand)
- Selects based on Priority (lower = better)
- **Uses Priority values**

> From AWS docs: *"EC2 Fleet uses priority to determine which launch template override to use first in fulfilling On-Demand capacity."*
...
```go
// From cloudprovider/types.go:369-384
...
|-----------|------------|------|------|
| Max instance types | karpenter-provider-aws | `pkg/providers/instance/instance.go` | 63 |
| Priority assignment | karpenter-provider-aws | `pkg/providers/instance/instance.go` | 476 |
| Allocation strategy | karpenter-provider-aws | `pkg/providers/instance/types.go` | 209-217 |
| Price adjustment | karpenter | `pkg/cloudprovider/types.go` | 369-384 |
| Instance type sorting | karpenter | `pkg/cloudprovider/types.go` | 221-233 |
| Instance type truncation | karpenter | `pkg/cloudprovider/types.go` | 322-327 |
| AMI to instance mapping | karpenter-provider-aws | `pkg/providers/amifamily/ami.go` | 228-241 |
| Launch template resolution | karpenter-provider-aws | `pkg/providers/amifamily/resolver.go` | 135-197 |
