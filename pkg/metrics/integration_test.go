...
func TestMetricsIntegration_NewMetricsCreatesAllFields(t *testing.T) {
	...
	assert.NotNil(t, m.SavingsPlanUtilizationPercent, "SavingsPlanUtilizationPercent should not be nil")
	assert.NotNil(t, m.SavingsPlanRemainingCapacityDollars, "SavingsPlanRemainingCapacityDollars should not be nil")
	...
	assert.NotNil(t, m.Info, "Info should not be nil")
}

// TestMetricsIntegration_SavingsPlanMetrics tests SP utilization and capacity metrics.
func TestMetricsIntegration_SavingsPlanMetrics(t *testing.T) {
	m := newTestMetrics(t)

	// Test compute SP (global)
	m.SetSavingsPlanMetrics("compute", "", "", 85.5, 100.25)

	// Test EC2 instance SP (family/region specific)
	m.SetSavingsPlanMetrics("ec2_instance", "m5", "us-west-2", 92.0, 50.0)
	m.SetSavingsPlanMetrics("ec2_instance", "c5", "us-east-1", 78.5, 200.0)

	// Verify compute SP metrics
	computeUtil := testutil.ToFloat64(m.SavingsPlanUtilizationPercent.WithLabelValues("compute", "all", "global"))
	assert.Equal(t, 85.5, computeUtil)

	computeCap := testutil.ToFloat64(m.SavingsPlanRemainingCapacityDollars.WithLabelValues("compute", "all", "global"))
	assert.Equal(t, 100.25, computeCap)

	// Verify EC2 instance SP metrics
	m5Util := testutil.ToFloat64(m.SavingsPlanUtilizationPercent.WithLabelValues("ec2_instance", "m5", "us-west-2"))
	assert.Equal(t, 92.0, m5Util)
}

// TestMetricsIntegration_InfoMetric tests the info metric with version and mode.
func TestMetricsIntegration_InfoMetric(t *testing.T) {
	m := newTestMetrics(t)

	// Set config which also sets the info metric
	m.SetConfigMetrics(false, 95.0)

	// Verify info metric is set
	infoValue := testutil.ToFloat64(m.Info.WithLabelValues(veneermetrics.Version, "false"))
	assert.Equal(t, float64(1), infoValue)

	// Test with disabled mode
	m2 := newTestMetrics(t)
	m2.SetConfigMetrics(true, 90.0)
